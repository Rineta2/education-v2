...
import { siswaSchema } from "@/hooks/schema/register/siswa/Schema"
...
import { SiswaInterface } from "@/hooks/schema/register/siswa/interfaceSiswa"
...
export default function Siswa() {
    ...
    const onSubmit = async (data: SiswaForm) => {
        try {
            ...
            const siswaData: SiswaInterface = {
                ...dataWithoutPassword,
            ...
            await setDoc(doc(db, collectionPath, userCredential.user.uid), siswaData);
            ...
            <div className='container mx-auto px-0 py-0 md:px-4 md:py-6 grid grid-cols-1 lg:grid-cols-2 gap-8'>
                ...
                <div className="px-4 lg:px-0 p-4 lg:p-0 flex flex-col gap-6 w-full">
                    ...
                    <div className="flex flex-col gap-2">
                        <h3 className="text-4xl md:text-5xl font-bold mb-2">Daftar Siswa</h3>
                        <p className="text-gray-600 text-[14px] md:text-[18px]">Masukkan data diri anda untuk mendaftar sebagai siswa.</p>
                    </div>
                        ...
                        <form onSubmit={handleSubmit(onSubmit)}>
                            <div className="container grid grid-cols-1 gap-6">
                                <div className="w-full min-h-[4rem]">
                                    {errors.nama && (
                                        <span className="text-red-500 text-sm mb-1">{errors.nama.message}</span>
                                    )}
                                    <label className="input input-bordered flex items-center gap-2 h-16">
                                        ...
                                        <input type="text" className="grow text-background placeholder:text-background:opacity-70 text-[16px]" placeholder="Nama Lengkap" {...register('nama')} />
                                    </label>
                                </div>
                                ...
                                <div className="w-full min-h-[4rem]">
                                    {errors.kelas && (
                                        <span className="text-red-500 text-sm mb-1">{errors.kelas.message}</span>
                                    )}
                                    <select className="select select-bordered w-full h-16 text-background:opacity-70 text-[16px] text-background" defaultValue="" {...register('kelas')}>
                                        <option value="" disabled>Pilih Kelas</option>
                                        <option className='text-background text-[16px]'>X-A</option>
                                        <option className='text-background text-[16px]'>X-B</option>
                                        <option className='text-background text-[16px]'>X-C</option>
                                        <option className='text-background text-[16px]'>XI-A</option>
                                        <option className='text-background text-[16px]'>XI-B</option>
                                        <option className='text-background text-[16px]'>XI-C</option>
                                        <option className='text-background text-[16px]'>XII-A</option>
                                        <option className='text-background text-[16px]'>XII-B</option>
                                        <option className='text-background text-[16px]'>XII-C</option>
                                    </select>
                                </div>
                                ...
                                <div className="w-full">
                                    {errors.no_telepon && (
                                        <span className="text-red-500 text-sm mb-1">{errors.no_telepon.message}</span>
                                    )}
                                    <label className="input input-bordered flex items-center gap-2 h-16">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="h-5 w-5 opacity-70 text-background">
                                        ...
                                        <input type="tel" className="grow text-background placeholder:text-background:opacity-70 text-[16px]" placeholder="Nomor HP" {...register('no_telepon')} />
                                    </label>
                                </div>
